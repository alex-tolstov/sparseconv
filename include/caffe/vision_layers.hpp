...
class BaseConvolutionLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  void forward_cpu_sparse_gemm(const Dtype* input, const Dtype *weightsCompressed, const int *indicesX, 
		const int *indicesY, const int sizeNonZeroWeights, Dtype *output, bool skip_im2col=false);
		
  void forward_cpu_sparse_conv(const Dtype* input, const Dtype* weightCompressed, const int* indicesX, 
		const int* indicesRow, const int sizeNonZeroWeigths, Dtype *output);

  void forward_cpu_sparse_conv_ch(const Dtype* input, const Dtype* weightCompressed, const int* indicesX, const int* indicesChannel, 
		const int* indicesRow, const int sizeNonZeroWeigths, Dtype *output);
...
template <typename Dtype>
class CConvolutionLayerTesting : public BaseConvolutionLayer<Dtype> {
 public:

  explicit CConvolutionLayerTesting(const LayerParameter& param)
      : BaseConvolutionLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "CConvolutionTesting"; }
