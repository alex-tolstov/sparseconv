...
void run20SeveralOutputs(const Dtype *img, const int imgSizeX, const int imgSizeY, Dtype *output, const int outputPitchX,
          const int outputSizeX, const int outputSizeY);

template<>
void run20SeveralOutputs<double>(const double *img, const int imgSizeX, const int imgSizeY, double *output, const int outputPitchX,
                  const int outputSizeX, const int outputSizeY) {
    throw 1;
}
...
template<>
void run20SeveralOutputs(const float *input, const int imgSizeX, const int imgSizeY, float *output,
                         const int outputPitchX, const int resSizeX, const int resSizeY) {
        ...
        const int inputSize = imgSizeX * imgSizeY;
        {
            ...
            for (int dim = 0; dim < resSizeX / 8; dim++) {
                ...
                for (int inputRow = 0; inputRow < imgSizeY; inputRow++) {
                    ...
                    if (realRowIdx >= 0) {
                        _mm256_storeu_ps(
                                output + outputChannelIdx * outputPitchX * resSizeY + outputPitchX * realRowIdx + shift,
                                _mm256_setzero_ps());
                    }
                }
            }
