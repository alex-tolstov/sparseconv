  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    if (strcmp(layers_[target_layer_id]->type(),"CInnerProduct")==0 || strcmp(layers_[target_layer_id]->type(),"CConvolution" )==0 ) {
      if(target_blobs.size() > source_layer.blobs_size()) {
		LOG(INFO) << "GOING TO COPY BLOBS FOR LAYER = " << layers_[target_layer_id]->type();
		LOG(INFO) << "NUMBER OF BLOBS = " << source_layer.blobs_size();
        for (int j = 0; j < source_layer.blobs_size(); ++j) {
          const bool kReshape = false;
          target_blobs[j]->FromProto(source_layer.blobs(j), kReshape);
          const Dtype* weight = target_blobs[j]->cpu_data();
          double sum = 0.0;
          for (int idx = 0; idx < target_blobs[j]->count(); idx++) {
			  sum += weight[idx];
		  }
		  LOG(INFO) << "layer index = " << j << ", sum of values in blob = " << sum;
        }
        continue;
      }
    }     
